...
interface Props {
  params: Promise<{ graph: string; slug: string }>;
}
...
export default async function NodePage({ params }: Props) {
      ...
      <>
        ...
        <main className="max-w-3xl mx-auto px-6 pt-16 pb-32">
          ...
          <div className="flex gap-2 flex-wrap">
            <Link
              href={gPath(graph, `/new?type=question&title=${encodeURIComponent(slug)}`)}
            ...
            <Link
              href={gPath(graph, `/new?type=thought&title=${encodeURIComponent(slug)}`)}
              className="px-3 py-1.5 rounded bg-sky-700 hover:bg-sky-600 flex items-center gap-2"
            >
              <NodeIcon type="thought" className="w-4 h-4" /> Create as Thought
            </Link>
            <Link
              href={gPath(graph, `/new?type=reference&title=${encodeURIComponent(slug)}`)}
              className="px-3 py-1.5 rounded bg-sky-700 hover:bg-sky-600 flex items-center gap-2"
            >
  ...
  const html = await renderMarkdown(graph, node.body_md || '_(empty)_');
  const comments = listComments(graph, slug);
  ...
  async function pin() {
    'use server';
    await requireAuth();
    togglePin(graph, slug);
    revalidatePath(gPath(graph, `/n/${slug}`));
    revalidatePath(gPath(graph));
  }
  ...
  async function deleteCommentAction(formData: FormData) {
    ...
    if (Number.isFinite(id)) deleteComment(graph, id);
    revalidatePath(gPath(graph, `/n/${slug}`));
  }
      ...
      <main className="max-w-3xl mx-auto px-6 pt-16 pb-32">
        <div className="flex items-start gap-2 mb-3 text-sm text-neutral-400 [html.light_&]:text-neutral-600">
          ...
          <div className="flex-1 flex items-center gap-2 flex-wrap justify-end">
            ...
            <Link
              href={gPath(graph, `/graph?focus=${encodeURIComponent(slug)}`)}
        ...
        {node.pdf_sha256 && (
          <div className="mt-6">
            <div className="flex items-center justify-between mb-2">
              ...
              <a
                href={gPath(graph, `/api/uploads/${node.pdf_sha256}`)}
                target="_blank"
                rel="noopener noreferrer"
            ...
            <PdfPreview src={gPath(graph, `/api/uploads/${node.pdf_sha256}`)} />
          </div>
        )}
        <ReactionBar graph={graph} slug={slug} initial={reactions} />
        <CommentsSection
          graph={graph}
          slug={slug}
          comments={comments}
        ...
        />
        <RelatedSection graph={graph} slug={slug} />
      </main>
      <BottomDock graph={graph} fromSlug={slug} />
    </>
  );
}
