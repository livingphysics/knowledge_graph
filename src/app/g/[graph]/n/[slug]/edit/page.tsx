...
export default async function EditNodePage({ params }: Props) {
  ...
  async function save(formData: FormData) {
    ...
    if (honeypot) redirect(gPath(graph));
    ...
    updateNode(graph, { slug, title, body_md, url, pdf_sha256, pdf_arxiv_id, bibtex_override, authorIp: ip });
    // If the new title changes the slug, rename everywhere and redirect to the new URL.
    const finalSlug = reslugFromTitle(graph, slug);
    redirect(gPath(graph, `/n/${finalSlug}`));
  }
      ...
      <main className="max-w-2xl mx-auto px-6 pt-16 pb-24">
        ...
        <form action={save} className="flex flex-col gap-4">
          ...
          <label className="flex flex-col gap-1">
            ...
            <MarkdownEditor graph={graph} name="body_md" defaultValue={node.body_md} rows={20} />
          </label>
